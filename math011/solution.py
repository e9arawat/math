...
def solver(numgrid, n: int):
    ...
    """
    max_product = 0
    row, column = len(numgrid), len(numgrid[0])
    for i in range(row):
        for j in range(column):
            if (j - n + 1) >= 0:
                result = 1
                for k in range(0, n):
                    result = result * numgrid[i][j - k]
                max_product = max(max_product, result)

            if (i - n + 1) >= 0:
                result = 1
                for k in range(0, n):
                    result = result * numgrid[i - k][j]
                max_product = max(max_product, result)

            if (i - n + 1) >= 0 and (j - n + 1) >= 0:
                result = 1
                for k in range(0, n):
                    result = result * numgrid[i - k][j - k]
                max_product = max(max_product, result)

            if (i - n + 1) >= 0 and (j + n - 1) < column:
                result = 1
                for k in range(0, n):
                    result = result * numgrid[i - k][j + k]
                max_product = max(max_product, result)
